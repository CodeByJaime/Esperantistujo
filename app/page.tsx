...
function renderTextWithBold(text: string) {
  ...
  return parts.map((part, index) => {
    if (index % 2 === 0) {
      ...
      return <span key={`${part.replace(/\s+/g, '-')}-${index}`} className="text-esperanto-verda font-semibold">{part}</span>;
    }
  });
}
...
export default function Home() {
  ...
  return (
    <div className="bg-[#0a0a0a] text-white min-h-screen font-serif">
      ...
      <section className="relative min-h-[92vh] flex flex-col justify-center overflow-hidden star-pattern">
        ...
        <div className="relative max-w-5xl mx-auto px-6 sm:px-10 py-24 sm:py-32">
          ...
          <h1 className="font-display animate-fade-up-2 text-5xl sm:text-7xl lg:text-8xl font-black leading-[0.95] tracking-tight mb-8">
            ...
            <span className="block text-white">Unu urbo.</span>
            <span className="block italic text-esperanto-verda">Unu estonteco.</span>
          </h1>

          {/* Subtitle */}
          <p className="animate-fade-up-3 font-sans text-gray-400 text-base sm:text-lg max-w-xl leading-relaxed mb-12">
            Esperantistujo estas koncepto, pri kiu multaj esperantistoj revis ekde kiam Zamenhof kreis la lingvon. Nu, ni opinias, ke ni estas en la perfekta momento por komenci krei Esperantistujon.
          </p>
      ...
      {/* MANIFESTO */}
      <section id="manifesto" className="max-w-5xl mx-auto px-6 sm:px-10 py-24 sm:py-32">
        <AnimatedSection>
                ...
                <div className="relative bg-white/2 backdrop-blur-sm border border-white/10 rounded-2xl p-8 sm:p-10 hover:border-esperanto-verda/20 transition-all duration-300">
                  ...
                  <div className="text-gray-300 font-sans text-base sm:text-lg leading-relaxed space-y-4">
                    {section.content.split('\n\n').map((paragraph) => (
                      <div key={`paragraph-${paragraph.slice(0, 20).replace(/\s+/g, '-')}-${section.number}`} className="mb-4">
                        {paragraph.split('\n').map((line) => {
                          if (line.trim().startsWith('1.') || line.trim().startsWith('2.') || line.trim().startsWith('3.') || line.trim().startsWith('4.')) {
                            return (
                              <div key={`numbered-${line.slice(0, 20).replace(/\s+/g, '-')}-${section.number}`} className="mb-3">
                                {renderTextWithBold(line)}
                              </div>
                            );
                          } else if (line.trim().startsWith('-')) {
                            return (
                              <div key={`bullet-${line.slice(0, 20).replace(/\s+/g, '-')}-${section.number}`} className="mb-2 flex items-start gap-2">
                                <span className="text-esperanto-verda mt-1">•</span>
                            ...
                            return (
                              <p key={`line-${line.slice(0, 20).replace(/\s+/g, '-')}-${section.number}`} className="mb-2">
                                {renderTextWithBold(line)}
                              </p>
                            );
                          }
      ...
      <footer className="border-t border-white/10 px-6 sm:px-10 py-10">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <span className="font-display font-bold text-white/60 text-sm">Esperantistujo</span>
          <span className="text-white/30 text-xs font-sans">
            © {new Date().getFullYear()} — Konstruita kun 💚 en Esperanto
          </span>
          <div className="flex gap-6">
            {NAV_LINKS.map((l) => (
              <a key={l.href} href={l.href} className="text-white/40 hover:text-white text-xs font-sans transition-colors">
                {l.label}
              </a>
            ))}
          </div>
